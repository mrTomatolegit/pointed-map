const INSERT = 'insert',
    ...
    setOMeta = (target, oMeta) => {
        Object.defineProperty(target, oMetaKey, {
            enumerable: false,
			configurable: true,
            value: oMeta
        });
    ...
    proxiedSplice = function proxiedSplice() {
        ...
        for (index = 0; index < removed; index++) {
            if (index < inserted) {
                ...
                changes.push(
                    new Change(DELETE, [startIndex + index], undefined, spliceResult[index], this)
                );
            }
        }
...
class OMetaBase {
    ...
    processOptions(options) {
        if (options) {
            ...
            if (invalidOptions.length) {
                throw new Error(
                    `'${invalidOptions.join(', ')}' is/are not a valid Observable option/s`
                );
            }
...
const Observable = Object.freeze({
    from: (target, options) => {
        if (!target || typeof target !== 'object') {
            ...
            return new ArrayOMeta({ target: target, ownKey: null, parent: null, options: options })
                .proxy;
            ...
            return new ObjectOMeta({ target: target, ownKey: null, parent: null, options: options })
                .proxy;
        }
    },
    isObservable: input => {
...
module.exports = { Observable, ObjectObserver };
